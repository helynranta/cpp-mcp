...
int main() {
    ...
    std::thread server_thread([&srv]() { srv.start(true); });
    ...
    client.set_progress_handler([](const progress_notification& notif) {
        ...
        if (notif.total.has_value()) {
            ...
            std::cout << "(" << static_cast<int>(notif.progress) << "/" << static_cast<int>(notif.total.value())
                      << ") ";
        } else {
    ...
    try {
        json params = {{"steps", 5}, {"step_duration_ms", 200}, {"_meta", {{"progressToken", "operation-001"}}}};
    ...
    try {
        json params = {{"steps", 3}, {"step_duration_ms", 150}};
