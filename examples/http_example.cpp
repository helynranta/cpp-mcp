...
int main() {
    ...
    server->register_get("/hello", [](const mcp::http::request_data& req, mcp::http::response_builder& res) {
        ...
        // Build JSON response
        json response = {{"message", "Hello, " + name + "!"}, {"timestamp", std::time(nullptr)}};
    ...
    server->register_post("/echo", [](const mcp::http::request_data& req, mcp::http::response_builder& res) {
        ...
        try {
            ...
            json response = {{"echo", request_body},
            ...
            json error = {{"error", "Invalid JSON"}, {"details", e.what()}};
    ...
    server->register_post("/calculate", [](const mcp::http::request_data& req, mcp::http::response_builder& res) {
        ...
        try {
            ...
            if (!request_body.contains("operation") || !request_body.contains("a") || !request_body.contains("b")) {
            ...
            json response = {{"result", result}, {"operation", operation}, {"operands", {a, b}}};
    ...
    std::cout << "\n--- Test 3: POST /echo ---\n";
    json echo_data = {{"message", "Hello from Beast client!"}, {"timestamp", std::time(nullptr)}};
    ...
    json calc_data = {{"operation", "add"}, {"a", 10}, {"b", 5}};
