...
int main() {
    ...
    tool increment_counter =
        tool_builder("increment_counter").with_description("Increments a counter for the current session").build();
    ...
    srv.register_tool(increment_counter, [&srv](const json& params, const std::string& session_id) -> json {
        ...
        return {{"counter", counter}, {"message", "Counter incremented successfully"}};
    });

    // Register a tool that retrieves session state
    tool get_state = tool_builder("get_state").with_description("Gets the current session state").build();
    ...
    srv.register_tool(get_state, [&srv](const json& params, const std::string& session_id) -> json {
        ...
        if (state.is_null() || state.empty()) {
            return {{"message", "No state data for this session"}, {"state", json::object()}};
        }

        return {{"message", "Session state retrieved successfully"}, {"state", state}};
    });
    ...
    srv.register_tool(set_user_data, [&srv](const json& params, const std::string& session_id) -> json {
        ...
        return {{"message", "Data stored successfully"}, {"key", key}, {"value", value}};
    });
