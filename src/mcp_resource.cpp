...
text_resource::text_resource(const std::string& uri, const std::string& name, const std::string& mime_type,
...
json text_resource::get_metadata() const {
    return {{"uri", uri_}, {"name", name_}, {"mimeType", mime_type_}, {"description", description_}};
}

json text_resource::read() const {
    modified_ = false;
    return {{"uri", uri_}, {"mimeType", mime_type_}, {"text", text_}};
}
...
binary_resource::binary_resource(const std::string& uri, const std::string& name, const std::string& mime_type,
                                 const std::string& description)
    : uri_(uri), name_(name), mime_type_(mime_type), description_(description), modified_(false) {}
...
json binary_resource::read() const {
    ...
    return {{"uri", uri_}, {"mimeType", mime_type_}, {"blob", base64_data}};
}
...
file_resource::file_resource(const std::string& file_path, const std::string& mime_type, const std::string& description)
    : text_resource("file://" + file_path, fs::path(file_path).filename().string(),
                    mime_type.empty() ? guess_mime_type(file_path) : mime_type, description),
      file_path_(file_path), last_modified_(0) {
    // Check if file exists
    if (!fs::exists(file_path_)) {
        throw mcp_exception(error_code::invalid_params, "File not found: " + file_path_);
    }
}
...
json file_resource::read() const {
    ...
    if (!file) {
        throw mcp_exception(error_code::internal_error, "Failed to open file: " + file_path_);
    }
