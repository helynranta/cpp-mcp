...
void sse_client::init_client(const std::string& scheme_host_port, bool validate_certificates,
                             const std::string& ca_cert_path) {
        ...
        while (sse_running_ && !stoken.stop_requested()) {
            try {
                ...
                auto res = sse_client_->get_stream(
                    sse_endpoint_, [&, this](const char* data, size_t data_length) -> bool {
...
json sse_client::send_jsonrpc(const request& req) {
    ...
    if (msg_endpoint_.empty()) {
        throw mcp_exception(error_code::internal_error,
                            "Message endpoint not set, SSE connection may not be established");
    }
    ...
    if (result.status_code / 100 != 2) {
        try {
            ...
            throw mcp_exception(error_code::parse_error, "Failed to parse JSON-RPC response: " + std::string(e.what()));
        }
    } else {
        ...
        if (status == std::future_status::ready) {
            ...
            if (response.contains("isError") && response["isError"].is_boolean() && response["isError"].get<bool>()) {
                if (response.contains("error") && response["error"].is_object()) {
                    ...
                    int code = err_obj.contains("code") ? err_obj["code"].get<int>()
                                                        : static_cast<int>(error_code::internal_error);
