...
streamable_http_client::streamable_http_client(const std::string& scheme_host_port, 
...
void streamable_http_client::init_clients(const std::string& scheme_host_port, 
                                         bool validate_certificates, 
                                         const std::string& ca_cert_path) {
    ...
    sse_thread_ = std::make_unique<std::jthread>([this](std::stop_token stoken) {
        ...
        while (sse_running_ && !stoken.stop_requested()) {
            try {
                ...
                auto res = sse_client_->get_stream(mcp_endpoint_, 
