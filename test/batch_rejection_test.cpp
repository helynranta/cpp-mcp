...
BOOST_AUTO_TEST_CASE(RejectsBatchRequestArray) {
    ...
    auto res = http_client->post("/mcp", {{"Content-Type", "application/json"}}, batch.dump(), "application/json");
    ...
    BOOST_CHECK(error_message.find("batch") != std::string::npos || error_message.find("array") != std::string::npos ||
...
BOOST_AUTO_TEST_CASE(AcceptsSingleRequest) {
    ...
    auto res = http_client->post("/mcp", {{"Content-Type", "application/json"}}, single_request.dump(),
                                 "application/json");
