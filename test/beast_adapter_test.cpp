...
BOOST_AUTO_TEST_CASE(SSEStreaming) {
    ...
    server->register_get("/sse", [](const request_data& req, response_builder& res) {
        ...
        res.set_chunked_content_provider("text/event-stream", [](size_t offset, streaming_data_sink& sink) -> bool {
            if (offset == 0) {
...
BOOST_AUTO_TEST_CASE(GetStreamRequest) {
    ...
    auto result = client.get_stream("/stream", [&](const char* data, size_t size) -> bool {
