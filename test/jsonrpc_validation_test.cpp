...
BOOST_AUTO_TEST_CASE(ValidRequestMessage) {
    json msg = {{"jsonrpc", "2.0"}, {"id", 1}, {"method", "test_method"}, {"params", {{"key", "value"}}}};
    std::string error;
    BOOST_CHECK(validate_request_message(msg, error));
}

BOOST_AUTO_TEST_CASE(ValidRequestWithoutParams) {
    json msg = {{"jsonrpc", "2.0"}, {"id", "req-1"}, {"method", "test_method"}};
    std::string error;
    BOOST_CHECK(validate_request_message(msg, error));
}

BOOST_AUTO_TEST_CASE(ValidNotificationMessage) {
    json msg = {{"jsonrpc", "2.0"}, {"method", "notifications/test"}};
    std::string error;
    BOOST_CHECK(validate_request_message(msg, error));
}

BOOST_AUTO_TEST_CASE(InvalidRequestMissingJsonRpc) {
    json msg = {{"id", 1}, {"method", "test_method"}};
    std::string error;
    BOOST_CHECK(!validate_request_message(msg, error));
...
BOOST_AUTO_TEST_CASE(InvalidRequestWrongJsonRpcVersion) {
    json msg = {{"jsonrpc", "1.0"}, {"id", 1}, {"method", "test_method"}};
    std::string error;
    BOOST_CHECK(!validate_request_message(msg, error));
...
BOOST_AUTO_TEST_CASE(InvalidRequestNullId) {
    json msg = {{"jsonrpc", "2.0"}, {"id", nullptr}, {"method", "test_method"}};
    std::string error;
    BOOST_CHECK(!validate_request_message(msg, error));
    BOOST_CHECK_NE(error.find("must not be null"), std::string::npos);
}
...
BOOST_AUTO_TEST_CASE(InvalidParamsNotStructured) {
    json msg = {
        {"jsonrpc", "2.0"}, {"id", 1}, {"method", "test_method"}, {"params", "invalid"} // String is not allowed
    };
    std::string error;
    BOOST_CHECK(!validate_request_message(msg, error));
...
BOOST_AUTO_TEST_CASE(ValidSuccessResponse) {
    json msg = {{"jsonrpc", "2.0"}, {"id", 1}, {"result", {{"key", "value"}}}};
    std::string error;
    BOOST_CHECK(validate_response_message(msg, error));
}

BOOST_AUTO_TEST_CASE(ValidErrorResponse) {
    json msg = {{"jsonrpc", "2.0"}, {"id", 1}, {"error", {{"code", -32600}, {"message", "Invalid Request"}}}};
    std::string error;
    BOOST_CHECK(validate_response_message(msg, error));
}
...
BOOST_AUTO_TEST_CASE(InvalidResponseMissingId) {
    json msg = {{"jsonrpc", "2.0"}, {"result", {{"key", "value"}}}};
    std::string error;
    BOOST_CHECK(!validate_response_message(msg, error));
...
BOOST_AUTO_TEST_CASE(InvalidErrorMissingCode) {
    json msg = {{"jsonrpc", "2.0"}, {"id", 1}, {"error", {{"message", "Invalid Request"}}}};
    std::string error;
    BOOST_CHECK(!validate_response_message(msg, error));
...
BOOST_AUTO_TEST_CASE(InvalidErrorMissingMessage) {
    json msg = {{"jsonrpc", "2.0"}, {"id", 1}, {"error", {{"code", -32600}}}};
    std::string error;
    BOOST_CHECK(!validate_response_message(msg, error));
...
BOOST_AUTO_TEST_CASE(InvalidErrorCodeNotInteger) {
    json msg = {{"jsonrpc", "2.0"}, {"id", 1}, {"error", {{"code", "not-an-int"}, {"message", "Invalid Request"}}}};
    std::string error;
    BOOST_CHECK(!validate_response_message(msg, error));
...
BOOST_AUTO_TEST_CASE(InvalidErrorMessageNotString) {
    json msg = {{"jsonrpc", "2.0"}, {"id", 1}, {"error", {{"code", -32600}, {"message", 123}}}};
    std::string error;
    BOOST_CHECK(!validate_response_message(msg, error));
