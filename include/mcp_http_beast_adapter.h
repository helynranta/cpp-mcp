...
public:
    explicit beast_response_builder(beast::http::response<beast::http::string_body>& response) : response_(response) {}

    void set_status(int code) override { response_.result(static_cast<beast::http::status>(code)); }

    void set_header(const std::string& name, const std::string& value) override { response_.set(name, value); }
    ...
    void set_chunked_content_provider(const std::string& content_type,
                                      std::function<bool(size_t offset, streaming_data_sink& sink)> provider) override {
    ...
    const std::function<bool(size_t, streaming_data_sink&)>& get_chunked_provider() const { return chunked_provider_; }
...
public:
    beast_server(bool use_ssl, const std::string& cert_path, const std::string& key_path)
        : use_ssl_(use_ssl), cert_path_(cert_path), key_path_(key_path), server_ready_(false), server_failed_(false),
          stopping_(false), active_connections_(0) {
        // SSL support will be added later
        if (use_ssl) {
            throw std::runtime_error(
                "SSL/TLS support not yet implemented for beast_server. Please disable SSL for now.");
        }
    }
    ...
    bool set_mount_point(const std::string& mount_point, const std::string& dir, const headers_map& headers) override {
    ...
    bool listen(const std::string& host, int port) override {
        try {
            ...
            server_thread_ = std::jthread(
                [this, host, port](std::stop_token stoken) { this->run_server(host, port, stoken); });
        ...
        // This is critical to prevent them from accessing freed member variables
        {
            ...
            while (active_connections_.load(std::memory_order_acquire) > 0) {
                if (connections_cv_.wait_until(lock, timeout) == std::cv_status::timeout) {
                    LOG_WARNING("Beast server: Timeout waiting for ", active_connections_.load(),
                                " connection(s) to complete");
                    break;
                }
            }
        }
...
private:
    ...
    void handle_connection(tcp::socket socket) {
        try {
            ...
            for (auto const& field : req) {
                req_data.headers.emplace(std::string(field.name_string()), std::string(field.value()));
            }
    ...
    void handle_chunked_response(tcp::socket& socket, const beast::http::request<beast::http::string_body>& req,
        ...
        try {
            ...
            while (!stopping_.load(std::memory_order_acquire) && builder.get_chunked_provider()(offset, sink)) {
                offset++;
            }
...
public:
    ...
    client_result get(const std::string& path) override {
        try {
            ...
            for (auto const& field : res) {
                result.headers.emplace(std::string(field.name_string()), std::string(field.value()));
            }
    ...
    client_result post(const std::string& path, const headers_map& headers, const std::string& body,
    ...
    void set_default_headers(const headers_map& headers) override { default_headers_ = headers; }
 ...
 */
inline std::unique_ptr<server_interface> create_beast_server(bool use_ssl = false, const std::string& cert_path = "",
